#!/usr/bin/env julia
# Timing benchmark: old CPU round-trip SC vs new in-place GPU SC
...
log_path = joinpath(@__DIR__, "..", "bench_sc_inplace_$(Dates.format(now(), "yyyymmdd_HHMMSS")).log")
...
logprintln("=" ^ 60)
logprintln("SC In-Place vs CPU Round-Trip Benchmark")
logprintln("=" ^ 60)
...
logprintln("SC offsets: seq=$(sc_offsets.seq), pair=$(sc_offsets.pair)")
...
# --- Run one phase ---
function run_phase(phase_name::String, use_inplace::Bool, batch_indices_all, global_idx_start)
    logprintln("\n--- Phase: $phase_name (inplace=$use_inplace) ---")
...
Random.seed!(123)
indices_old = [rand(1:length(all_proteins), BS) for _ in 1:N_BATCHES]
indices_new = [rand(1:length(all_proteins), BS) for _ in 1:N_BATCHES]

# --- Phase A: OLD (CPU round-trip) ---
result_old = run_phase("OLD_CPU_ROUNDTRIP", false, indices_old, 0)

# --- Phase B: NEW (in-place GPU) ---
result_new = run_phase("NEW_INPLACE_GPU", true, indices_new, result_old.global_idx)

# --- Summary ---
logprintln("\n" * "=" ^ 60)
logprintln("RESULTS (BS=$BS, $N_BATCHES batches each, $N_WARMUP warmup skipped)")
logprintln("=" ^ 60)
logprintln("  OLD (CPU round-trip): median=$(round(Int, result_old.median_ms)) ms, per-sample=$(round(result_old.per_sample, digits=1)) ms")
logprintln("  NEW (in-place GPU):   median=$(round(Int, result_new.median_ms)) ms, per-sample=$(round(result_new.per_sample, digits=1)) ms")

speedup = result_old.median_ms / result_new.median_ms
saved_pct = (result_old.median_ms - result_new.median_ms) / result_old.median_ms * 100
logprintln("  Speedup: $(round(speedup, digits=2))x")
logprintln("  Time saved: $(round(saved_pct, digits=1))%")
logprintln("\nDone!")
close(LOG_IO)
