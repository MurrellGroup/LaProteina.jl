#!/usr/bin/env julia
# Warmdown training for the triangle-update branching model.
# Loads from a checkpoint, does burnin → train → linear warmdown.
# Same schedules as train_branching_full_OU_tri.jl.
#
# Usage:
#   julia --project=/home/claudey/JuProteina/run -t 8 scripts/warmdown_branching_OU_tri.jl
#
# Environment variables:
#   CHECKPOINT - Path to checkpoint JLD2 (required or uses default)
#   SHARD_DIR - Directory containing precomputed shards
#   OUTPUT_DIR - Directory for outputs
#   BATCH_SIZE - Batch size (default: 4)
#   N_BATCHES - Total batches for this run (default: 20000)
#   WARMDOWN_BATCHES - Linear warmdown batches at end (default: 10000)
#   SAMPLE_EVERY - Checkpoint every N batches (default: 5000)
...
println("=" ^ 70)
println("Branching Flows Warmdown - Triangle Updates (OUBridgeExpVar)")
println("=" ^ 70)

# ============================================================================
# Configuration
# ============================================================================
checkpoint_path = get(ENV, "CHECKPOINT",
...
output_dir = get(ENV, "OUTPUT_DIR", joinpath(@__DIR__, "..", "outputs", "branching_OU_tri_warmdown_$(Dates.format(now(), "yyyymmdd_HHMMSS"))"))
...
# Learning rate schedule — exact same as train_branching_full_OU.jl
...
println("Model: ScoreNetwork with triangle updates")
println("Optimizer: Muon with burnin_learning_schedule(1e-5, 2.5e-4, 1.05, 0.99995)")
println("Log file: $log_file")

open(log_file, "w") do io
    println(io, "# Branching Flows Warmdown Log (OUBridgeExpVar + Triangle Updates)")
    println(io, "# Started: $(now())")
    println(io, "# Checkpoint: $checkpoint_path")
...
println("Warmdown starts at batch: $warmdown_start")
...
for (batch_idx, bd_cpu) in enumerate(dataloader)
    ...
    if batch_idx == 1 || batch_idx % sample_every == 0
        checkpoint_path_out = joinpath(output_dir, "checkpoints", @sprintf("checkpoint_batch%06d.jld2", batch_idx))
        model_cpu = cpu(model)
        save_branching_weights(model_cpu, checkpoint_path_out; include_base=true)
        println("  Saved checkpoint: $checkpoint_path_out")
    end
...
# ============================================================================
println("\n" * "=" ^ 70)
println("Warmdown Complete (Triangle Updates)")
println("=" ^ 70)
...
model_cpu = cpu(model)
final_path = joinpath(output_dir, "branching_tri_warmdown_final.jld2")
