...
println("=" ^ 70)
println("Branching Flows Training - Full Fine-tuning")
println("=" ^ 70)
...
# State loss scaling
# With zero_com fix, raw CA loss ~1.0, LL ~10.0, split/del ~0.5-1.0
# Scale so all components are in similar range post-scaling
ca_loss_scale = 2.0f0
ll_loss_scale = 0.1f0
...
open(log_file, "w") do io
    ...
    println(io, "# Loss scales: ca=$ca_loss_scale, ll=$ll_loss_scale")
...
for (batch_idx, bd_cpu) in enumerate(dataloader)
    ...
    # Write to log file (scaled losses)
    open(log_file, "a") do io
        @printf(io, "%d,%d,%.2e,%.4f,%.4f,%.4f,%.4f,%.4f,%.3f,%.3f,%.0f\n",
