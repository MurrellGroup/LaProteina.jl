#!/usr/bin/env julia
# Train Branching Flows model - NULL branching sanity check
# Same as train_branching_full.jl but with branching effectively disabled:
#   - deletion_pad = 1 (no extra deletion padding)
#   - length_mins = exact training sample lengths (no coalescence possible)
# The model should learn zero split/del rates and match standard la-proteina.
#
# Usage:
#   julia -t 8 scripts/train_branching_null.jl
#
...
#   N_BATCHES - Number of training batches (default: 10000)
...
println("=" ^ 70)
println("Branching Flows Training - NULL Branching Sanity Check")
println("=" ^ 70)
...
output_dir = get(ENV, "OUTPUT_DIR", joinpath(@__DIR__, "..", "outputs", "branching_null_$(Dates.format(now(), "yyyymmdd_HHMMSS"))"))
batch_size = parse(Int, get(ENV, "BATCH_SIZE", "4"))
n_batches = parse(Int, get(ENV, "N_BATCHES", "10000"))
...
# Branching parameters — NULL: no splits or deletions possible
deletion_pad = 1

# State loss scaling (to match split/del magnitude ~1)
# CA uses log schedule → τ much larger → 1/(1-τ)² scaling ~10x more aggressive
ca_loss_scale = 0.001f0
ll_loss_scale = 0.01f0
...
open(log_file, "w") do io
    println(io, "# Branching Flows NULL Training Log")
    ...
    println(io, "#")
    println(io, "# Columns: batch,shard,batch_in_shard,lr,total_loss,ca_loss,ll_loss,split_loss,del_loss,t_min,t_max,time_ms")
end
...
function prepare_training_batch(indices, proteins_ref, P_ref, X0_sampler, base_model)
    ...
    # Exact protein lengths as ints — no coalescence possible
    exact_lengths = [Int(sum(proteins_ref[i].mask .> 0.5)) for i in indices]
    ...
    # Per-modality schedule-transformed times for model conditioning
    # Bridge interpolates at τ(s) internally; model must also see τ(s)
    P_ca_ref = P_ref.P[1]
    P_ll_ref = P_ref.P[2]
...
for (batch_idx, bd_cpu) in enumerate(dataloader)
    ...
    loss, grads = Flux.withgradient(model) do m
        ...
        # Log raw losses (unscaled, unclamped) and t values
        Zygote.ignore() do
            ...
            # Norms to check if Xt_ca is degenerate (near zero)
    ...
    if batch_idx % 10 == 0
        ...
        if batch_idx == warmdown_start
            global sched
            sched = linear_decay_schedule(sched.lr, 0.000000001f0, warmdown_batches)
        end
    ...
    open(log_file, "a") do io
        @printf(io, "%d,%d,%d,%.2e,%.4f,%.2f,%.2f,%.4f,%.4f,%.3f,%.3f,%.0f\n",
                batch_idx, shard_idx, batch_in_shard, sched.lr,
    ...
    if batch_idx % 100 == 0 || batch_idx == 1
        ...
        @printf("Batch %6d [shard %2d]: loss=%.3f (ca=%.1f, ll=%.1f, split=%.3f, del=%.3f), lr=%.2e%s, time=%.0fms\n",
